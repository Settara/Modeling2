...
namespace Laba_2
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            Toj = new double[N];
            g = CreateGraphics();
            string filePath = Path.Combine("..", "..", "inputfile.txt"); //Указываем путь до файла с входными данными
            ArrayLoader.LoadArrayFromFile(filePath, initialDataArray);
            FillTable(initialDataArray);
        }
        
        //Этот метод отображает содержимое таблицы входных данных
        ...
        //Этот метод выполняет вычисление параметров лямбда i
        public void CalcLI()
        {
            for (int i = 0; i < N; i++)
            {
        ...
        //Этот метод выполняет вычисление параметров Pi1 и Pi2
        public void CalcPI()
        {
            double ot, to;
            if (N % 2 == 0)
        ...
        //Этот метод отвечает за 1 правило Петрова
        public List<int> Rule1()
        {
            ...
            D2.Sort((x, y) =>
            {
                ...
                if (result == 0)
                {
                    return -x[2].CompareTo(y[2]); //Тут не используем отрицание
                }
                return result;
            });
            D2.Reverse();
        ...
        //Этот метод отвечает за 2 правило Петрова
        ...
        //Этот метод отвечает за 3 правило Петрова
        ...
        //Этот метод отвечает за 4 правило Петрова
        ...
        //Обработчик кнопки работающей со 1 правилом Петрова
        ...
        //Обработчик кнопки работающей со 2 правилом Петрова
        ...
        //Обработчик кнопки работающей с 3 правилом Петрова
        ...
        //Этот метод добавляет в таблицу информацию о времени простоя и времени ожидания
        ...
        //Этот метод метод заполняет первый столбик второй таблицы, в котором указаны номера деталей.
        ...
        //Этот метод метод добавляет в таблицу информацию о результирующем порядке обработки деталей
        ...
        //Обработчик кнопки работающей с 4 правилом Петрова
        ...
        //Обработчик кнопки, который запускает метод перебора
        private void buttonEnumerate_Click(object sender, EventArgs e)
        {
            CalcPI();
            CalcLI();
            List<int> posled = Enumerate();
        ...
        //Этот метод генерирует все возможные перестановки
        ...
        //Этот метод генерирует случайную перестановку
        ...
        private void buttonStartSequence_Click(object sender, EventArgs e)
        {
            ...
            //Используем ба зовую последовательность от 1 до N
